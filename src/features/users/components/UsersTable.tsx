...
export function UsersTable() {
  ...
  const { data: users, isLoading } = useUsers({ search: debouncedSearch, status, plan })
  ...
  function exportCsv() {
    ...
    const rows = sorted.map(u => [u.name, u.email, u.plan, u.status, u.mrr, u.joinedAt])
  ...
  const SortIcon = ({ k }: { k: SortKey }) => (
    <i className={`ti ti-chevron-${sortKey === k ? (sortDir === "asc" ? "up" : "down") : "up"} ml-1 opacity-${sortKey === k ? "100" : "20"} text-xs`} aria-hidden="true" />
  )
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide cursor-pointer hover:text-gray-600" onClick={() => toggleSort("name")}>
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide cursor-pointer hover:text-gray-600" onClick={() => toggleSort("mrr")}>
                MRR <SortIcon k="mrr" />
              </th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide cursor-pointer hover:text-gray-600" onClick={() => toggleSort("joinedAt")}>
                Joined <SortIcon k="joinedAt" />
              </th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide cursor-pointer hover:text-gray-600" onClick={() => toggleSort("lastActive")}>
          ...
          <tbody>
            {isLoading
              ...
              : sorted.map((user, i) => (
                <tr key={user.id} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${i === sorted.length - 1 ? "border-0" : ""}`}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-gray-800 font-medium leading-none">{user.name}</p>
        ...
        {!isLoading && sorted.length > 0 && (
          <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-400">
              Total MRR: <span className="font-medium text-gray-600">{formatCurrency(sorted.reduce((s, u) => s + u.mrr, 0))}</span>
            </p>
          </div>
        )}
      </div>
